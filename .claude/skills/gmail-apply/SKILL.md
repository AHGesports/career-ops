---
name: gmail-apply
description: Per-portal application automation. V3 architecture — Node orchestrator drives URL loop deterministically, spawns ONE haiku worker per URL with task-specific contract (External / Validate / Recover). Two modes — (1) SINGLE-URL when user passes a job URL → run scripts/gmail-apply.mjs against that URL, parent (opus) takes over via MCP if needed; (2) BATCH when no URL → orchestrator iterates queue, opus only consults on disputed or missing-evidence cases. Eligible rows status Evaluated or Auto-Match. Triggers — single — /gmail-apply <URL>, /apply-portal <URL>, "apply to <URL>"; batch — /gmail-apply (no URL), "batch apply", "apply to all evaluated", "apply to auto-matched". No PDF or report generation.
user_invocable: true
...
Node orchestrator does the URL iteration. Haiku worker handles MCP for ONE URL at a time. Opus (this skill) only steps in when script + worker claims disagree OR no evidence file produced.

## Mode pick

| Input | Mode | Driver |
|---|---|---|
| First positional starts with `http(s)://` | **single** | Direct script + opus MCP if needed |
...
| `--autofix` | ✓ | ✓ | After Applied via Recover branch, surface yaml-patch candidates for opus to apply. |
...
- DO NOT spawn `Agent` subagent. Batch spawns its own `claude -p` haiku workers from inside Node.
...
|---|---|
| `redirect.detected:true` | External ATS handover. Parent agent (opus) runs External flow per `scripts/escalation-ladder.md` § "External ATS handover". CV path absolute. |
...
2. Pick task type from script outcome:
   ...
   - else → Recover worker
3. Spawn haiku worker with one URL + task-specific context. Worker writes `data/batch-runs/<run_id>/evidence/<slug>-worker-<ts>.json` and emits stdout JSON with `claim`.
4. Compare claims:
...
5. With `--autofix` and Applied via Recover: log `applied_via_mcp_recovery` for opus to patch yaml.
...
**If `disputed.length > 0`**: opus referees each. For each entry:
...
**If `no_evidence.length > 0`**: opus uses chrome-devtools MCP DIRECTLY to probe each URL's live tab.
...
**Use opus MCP ONLY in the no_evidence path.** All other URLs already adjudicated by Node.

### Step 3.5 — Mailto handler (opus-side)
...
disputed (opus-resolved): 2 → 1 Applied, 1 Failed, 0 escalated.
no_evidence (opus MCP probe): 1 → 1 Applied.
mailto (opus sent): 1 → 1 Applied via email.
...
Canonical at **`scripts/escalation-ladder.md`** — tab safety, JS templates, success determination, external-ATS markers + handover. Read once when running External handover (single mode) or doing opus-side disputed/no_evidence resolution.
...
Triggered when URL ends Applied via Recover branch (script Failed → worker MCP-fixed). Orchestrator surfaces `applied_via_mcp_recovery` log entry; opus patches yaml.
...
- `evidence/<slug>-worker-<ts>.json` — **NEW V3 — haiku worker page-state evidence (Validate/Recover/External)**
- `disputed-urls.ndjson` — opus-side review queue (only ambiguous-after-opus cases)
...
| Worker output not JSON | Same — logged as `no_evidence`, opus MCP probes the tab. |
